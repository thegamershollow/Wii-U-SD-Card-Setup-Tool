...
aromaBase = (
    "https://github.com/wiiu-env/Aroma/releases/download/beta-14/aroma-beta-14.zip"
)
...
if sdVerify != True:
    giveSdPath = input(
        "Please specify the path of your "
        + Fore.CYAN
        + "Wii U"
        + Fore.RESET
        + " SD Card: "
...
if prompt == "1":
    ...
    shutil.move(
        sd + "/manifest.install", sd + "/wiiu/apps/appstore/.get/packages/appstore"
    )
...
if prompt == "2":
    ...
    pkgTable = pkgTable.reindex(
        columns=["name", "author", "category", "version", "filesize"]
    )
    pkgTable.rename(
        columns={
            "category": "Category",
            "version": "Version",
            "filesize": "Download Size(KB)",
            "app_dls": "App Downloads",
            "author": "Author",
            "updated": "Update Date",
            "name": "App Name",
        },
    ...
    hbSelect = input(
        "Type the app/s name/ to download it "
        + Fore.LIGHTCYAN_EX
        + "**if multiple are selected this process will take a lot longer**"
        + Fore.RESET
        + "\n\nSeperate the app names with commas if you want to download multiple apps at once.\n\nSelection: "
    )
    hbSelect = hbSelect.split(",")
    ...
    for item in hbSelect:
        if item in apps:
            ...
            if dlPath != True:
                ...
                shutil.move(
                    sd + "/manifest.install",
                    sd + "/wiiu/apps/appstore/.get/packages/" + item,
                )
                shutil.move(
                    sd + "/info.json", sd + "/wiiu/apps/appstore/.get/packages/" + item
                )
...
if prompt == "4":
    ...
    oscTable = oscTable.reindex(
        columns=["internal_name", "category", "version", "short_description", "coder"]
    )
    oscTable.rename(
        columns={
            "internal_name": "App Name",
            "category": "Catergory",
            "version": "Version",
...
if prompt == "5":
    ...
    warn = input(
        ...
        + "*⚠️WARNING⚠️* "
        + Fore.RESET
        + "This will delete/remove all files from the SD card\nContinue (Y/N):\n"
    )
    if warn == "Y" or warn == "y":
